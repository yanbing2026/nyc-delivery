...
```bash
./run_tests.sh          # 一次跑完 6 个套件，共 289 项检查
```
...
cd docs    && node test-order-page-static.js # 46 项：点单页（站点首页）—— 后端用桩，不依赖线上地址服务，结果确定
cd worker  && node test_worker.mjs           # 93 项：Worker 全链路（真实 SQL + 真实地址/路线 + App 发布菜单/下单校验）
```

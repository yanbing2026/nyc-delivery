...
## 四点五、老客取回与线上顾客建档（2026-09-16 加）

**顾客侧**：点单页有个「老客？输手机号带出上次的资料」。输号码 → 带出姓名/最近地址 + "来过 N 次"
+ 上次点的菜（可一键"再来一单"，已经下架的菜会明确说"现在没有"，不会假装加上）。
同一个手机号换各种写法（`917-555-0123` / `(917) 555 0123`）都能认出来 —— 号码先归一化成纯数字再比。
页面还会把上次填的姓名/电话/地址记在手机上，下次打开自动填。

**店里侧**：每收到一笔线上单，App 会按手机号把顾客存进 POS 的客户列表（`customers` 表），
名字/地址更新成最近一次用的，**来过次数 +1**。手工在 POS 里存的顾客不会加这个次数。
号码不足 10 位不建档（宁可不存，也不要在客户列表里塞脏号码）。
数据库版本 28 → 29（老设备靠自愈迁移自动补 `orders_count` 列，不需要重装数据）。

**⚠️ 隐私说明（这是当初选的方案 A，要知道它的边界）**：
取回**不做任何验证** —— 知道某个手机号的人，就能看到那个号码的姓名和最近一次送餐地址。
之所以能接受：网站**只收现金**，没有卡号、没有支付信息、看不到订单历史明细（只回最近 3 单的
金额和单数，不回完整地址列表）。如果以后要在线付款，或者不想接受这个暴露面，
就升级成「手机号 + 4 位 PIN」或短信验证码（后者要 Twilio 费用 + 美国 10DLC 审核）。
...
| POST | `/api/lookup` | **老客取回**：只给手机号，回姓名 + 最近地址 + 来过次数 + 上一单的菜（给"再来一单"）。不做验证，见下面的隐私说明。同一 IP 60 次/小时 |
